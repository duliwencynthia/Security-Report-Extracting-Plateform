...
class RandomProjectionSynthesizer:
    def __init__(self, texts, labels, min_samples=100, synthetic_multiplier=2):
        self.texts = texts
        self.labels = labels
    ...
    def find_farthest_points(self, X):
        ...
        idx1 = np.argmax(sims.sum(axis=1))
        idx2 = np.argmin(sims[idx1])
        return idx1, idx2

    def split(self, X, texts, labels):
        idx1, idx2 = self.find_farthest_points(X)
        pivot1, pivot2 = X[idx1], X[idx2]
        line = pivot1 - pivot2
        projections = X.dot(line)
        median = np.median(projections)
        left_idx = projections <= median
        right_idx = projections > median
        return (X[left_idx], [texts[i] for i in np.where(left_idx)[0]], [labels[i] for i in np.where(left_idx)[0]]), \
               (X[right_idx], [texts[i] for i in np.where(right_idx)[0]], [labels[i] for i in np.where(right_idx)[0]])

    def mutate_text(self, text):
        words = text.split()
        if len(words) > 1:
            swap_idx = np.random.choice(len(words), 2, replace=False)
            words[swap_idx[0]], words[swap_idx[1]] = words[swap_idx[1]], words[swap_idx[0]]
        return ' '.join(words)

    def oversample(self):
        print("Applying Random Projection Oversampling...")
        X = self.vectorizer.fit_transform(self.texts).toarray()

        texts_new = list(self.texts)
        labels_new = list(self.labels)
        ...
        for label, count in label_counts.items():
            if count < self.min_samples:
                ...
                indices = [i for i, lab in enumerate(self.labels) if lab == label]
                X_label = X[indices]
                texts_label = [self.texts[i] for i in indices]
